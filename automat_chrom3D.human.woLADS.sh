...
find $dir -name '*40000.matrix' -exec cp {} $chrom3D  \;
find $dir -name '*1000000.matrix' -exec cp {} $chrom3D  \;
find $dir -name '*40000_abs.bed' -exec cp {} $chrom3D  \;
find $dir -name '*1000000_abs.bed' -exec cp {} $chrom3D  \;

matrix_intra=$chrom3D/*40000.matrix
abs_intra=$chrom3D/*40000_abs.bed
...
curl -s "http://hgdownload.cse.ucsc.edu/goldenPath/hg38/database/cytoBand.txt.gz" | gunzip -c | grep acen | pairToBed -a $chrom3D/intrachr_bedpe/sample_50kb.domain.RAW.bedpe -b stdin -type neither > $chrom3D/intrachr_bedpe/sample_50kb.domain.RAW.no_cen.bedpe

#(i) Calculate the P-value and odds ratio for each pair of TADs

$NCHG_dir/NCHG -m 40000 -p $chrom3D/intrachr_bedpe/sample_50kb.domain.RAW.no_cen.bedpe > $chrom3D/intrachr_bedpe/sample_50kb.domain.RAW.bedpe.out
